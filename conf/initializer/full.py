...
def format_loss_fields(config, precision=2):
    parts = []
    for name, weight in config.loss_weights.items():
        if weight == 0:
            continue

        s = f"{weight:.{precision}f}"
...
def get_exp_group(config):
    ...
    if hasattr(config, "active_losses"):
        loss_parts = format_loss_fields(config)
        if loss_parts:
            parts.append("-".join(loss_parts))
...
def get_loss(loss_weights: dict[str, float]) -> list[str]:
    active_losses = [name for name, w in loss_weights.items() if w > 0]
...
def init_config(config):
    ...
    config.active_losses = get_loss(config.loss_weights)
    ...
    config.instruction_csv_path = get_csv_path(config.instruction_csv)
