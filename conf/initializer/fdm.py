...
def get_game_name(game_str: str) -> list[str]:
    ...
    for k in keys:
        if k not in GAME_NAME_MAP:
            raise ValueError(f"Unknown game name: {k}. Available: {list(GAME_NAME_MAP.keys())}")
        games.append(GAME_NAME_MAP[k])
    return games


def init_config(config):
    """Initialise FDMConfig for training."""
    ...
    if config.vit_batch_size is None:
        ...
        logger.info(f"vit_batch_size not specified, using batch_size: {config.vit_batch_size}")
