...
from conf.initializer.full import init_config
...
from utils.logger import get_logger, select_render_samples, visualize_and_log, tsne_logger
...
from utils.checkpoint import CheckpointManager, DummyCheckpointManager

logger = get_logger(__file__)


def train(config: TrainConfig, tsne_queue_in=None, tsne_queue_out=None):
    ...
    clip_optimizer = torch.optim.AdamW(clip_model.parameters(), lr=config.clip_lr)
    clip_lr_scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(
        clip_optimizer,
        T_max=config.n_epochs,
        eta_min=config.clip_lr * 0.05,
    )
    ...
    vae_optimizer = torch.optim.AdamW(vae_model.parameters(), lr=config.vae_lr)

    vq_lr_scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(
        vae_optimizer,
        T_max=config.n_epochs,
        eta_min=config.vae_lr * 0.05,
    )
    ...
    # instantiate checkpoint manager (uses get_log_dir to determine paths)
    checkpoint_manager = CheckpointManager(
        log_dir_root=config.exp_path,
        config=config,
        save_interval=config.save_interval,
        save_keep=config.save_keep,
        logger=logger,
        get_log_dir_fn=get_log_dir,
    ) if config.save_ckpt else DummyCheckpointManager()

    # register models to the checkpoint manager (we only track models for models-only saves)
    ...
    vae_trainer = VAETrainer(model=vae_model,
                             ...
                             trainable_clip=config.trainable_clip,
    ...
    for epoch in range(config.n_epochs):
        ...
        # periodic checkpoint save (epoch is 1-based when calling)
        is_last_epoch = (epoch + 1) == config.n_epochs
        checkpoint_manager.maybe_save(epoch + 1, is_last_epoch=is_last_epoch)
...
@hydra.main(version_base=None, config_path="conf", config_name="train_full")
